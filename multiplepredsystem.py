...
diabetes_model = pickle.load(open('classmodel.sav','rb'))
heart_model = pickle.load(open('regmodel.sav','rb'))
parkinson_model = pickle.load(open('svmmodel.sav','rb'))
...
if (selected=='Heart Disease Prediction'):
    ...
    if st.button('Heart Disease Prediction result'):
        ...
        if(dia_pred[0]==1):
            heart_diag = 'The person have heart problem'
        else:
            heart_diag = 'The person doesnt have heart problem'
...
if (selected=='Parkinson Disease Prediction'):
    ...
    if st.button('Parkinson Disease Prediction result'):
        parkin_pred = parkinson_model.predict([[name,Fo,Fhi,Flo,Jitter_per,Jitter_abs,RAP,PPQ,DDP,Shimmer,Shimmer_dB,Shimmer_APQ3,Shimmer_APQ5,MDVP_APQ,Shimmer_DDA,NHR,HNR,status,RPDE,DFA,spread1,spread2,D2,PPE]])
        if(dia_pred[0]==1):
            parkin_diag = 'The person is attacked by parkinson disease'
        else:
            parkin_diag = 'The person is not affected by parkinson disease'
